...
let startPosition = 0;
let outDuration = 0;
const tweenedHeight = tweened(void 0, {
  duration: 300,
  easing: cubicOut
});
const clickOutside = (node) => {
  const handleClick = (event) => {
    ...
    if (!node.contains(target) && !target.hasAttribute("blockOutclick")) {
...
const touchMoveSlide = (e) => {
  ...
  if (direction === "up" && different > 0 && different <= startPosition && moveToClose) {
    container.style.transform = `translateY(${startPosition + different}px)`;
    if (different > defaultHeight / 2)
      dispatch("dragged-to-close");
  }
};
const touchEnd = () => {
  blockCuper = true;
